---
name: scan-secrets
description: Scan a repository for accidentally committed secrets like API keys, passwords, and private keys. Use when the user asks to scan for secrets, check for leaked credentials, or review security.
allowed-tools: Bash, Read, Grep, Glob
---

Scan the repository for secrets. If the user provided a target path via arguments, use that: $ARGUMENTS
...
1. Read the `patterns.txt` file from this skill's folder
...
1. **Replace hardcoded secrets**: Help rewrite code to use environment variables instead
...
4. **Clean git history** (explain only, let user execute): Recommend `git filter-repo` or BFG Repo-Cleaner
